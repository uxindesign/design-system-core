...
- **`.ds-code-block` regredido sem estilo** desde o refactor `d5c4fd2`: a regra CSS não foi migrada pra `docs/layout.css`, blocos JSON DTCG em foundations-* apareciam como prosa solta. Recolocada usando `--ds-background-inverse` + `--ds-content-inverse` (mesma paleta do `<pre>` global em `reset.css`). **Débito conhecido**: ambos flipam pra branco em dark mode — code blocks deveriam ser "always dark" (padrão universal de terminal/editor). Resolver via ADR + semantic novo `code.background.default` / `code.content.default`.
