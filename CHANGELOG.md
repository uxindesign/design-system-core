...
- **Modal Figma/CSS aplicado aos Component tokens anatômicos.** Criado `component.modal` para overlay, root, header, body, title, footer e close; as 3 variantes vivas do Figma (`Small`, `Medium`, `Large`) agora usam Component tokens nesses contratos. CSS passa a consumir `--ds-modal-*`, remove `z-index` hardcoded via `semantic.z.modal` e aplica `component.modal.root.shadow.default` em vez de depender da utility `.ds-elevation-4`.
...
- **Skeleton Figma/CSS aplicado aos Component tokens anatómicos.** Criado `component.skeleton.{text,circle,rectangle}` para height/size/radius dos 3 variants vivos do Figma; Text e Rectangle usam alturas anatómicas específicas (`12/120`) e Circle usa `semantic.size.xl` (`40`). CSS passa a consumir `--ds-skeleton-*`; documentação corrige background para `semantic.background.subtle` e remove mappings antigos de radius Foundation.
- **Spinner Figma/CSS aplicado aos Component tokens anatómicos.** Criado `component.spinner.root.size.{sm,md,lg}`, `component.spinner.root.radius.default` e `component.spinner.ring.stroke-width.default`; os 6 variants do Spinner no Figma agora usam Component tokens em root, Track e Indicator. CSS passa a consumir `--ds-spinner-*`; documentação corrige tamanhos `16/24/32` e motion para `duration.slower` + `ease.linear`.
- **Avatar Figma/CSS aplicado aos Component tokens anatómicos.** Criado `component.avatar.root.size.*`, `component.avatar.root.radius.default`, `component.avatar.initials.{font-size,line-height,font-weight}.*` e `component.avatar.icon.size.*`; os 6 variants do Avatar no Figma agora usam Component tokens para root e tipografia anatómica. CSS passa a consumir `--ds-avatar-*`, corrige icon sizes para `20/24/32` e documentação corrige `lg` de `56px` para `64px`.
...
- **Piloto Component materializado em Figma + JSON para Checkbox, Radio e Toggle.** Criada a collection Figma `Component` com modo `Default` e 22 variables `component/...`: target heights 32/40/48, box/control 16/20/24, track Toggle 28×16 / 44×24 / 56×32 e thumb inset 2px. Checkbox/Radio `Control` e Toggle `Track` foram rebindados às novas variables; `tokens/component/{checkbox,radio,toggle}.json`, registry, CSS consumidor e snapshot Figma foram atualizados.
- **Alturas Component para controles single-line.** Button, Input Text e Select ganharam `component.<componente>.root.height.{sm,md,lg}` aliasados a `semantic.size.{lg,xl,2xl}`. Figma foi rebindado no root do Button e no `Field` visual de Input/Select; CSS consumidor passou a usar `--ds-button-root-height-*`, `--ds-input-root-height-*` e `--ds-select-root-height-*`.
